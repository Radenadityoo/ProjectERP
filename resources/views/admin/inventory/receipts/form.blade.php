...
@section('content')
    <div class="p-6">
        <h1 class="text-2xl font-semibold mb-4">New Receipt</h1>

        <form method="post" action="{{ route('admin.inventory.receipts.store') }}">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="block text-sm">Reference</label>
                    <input name="reference" class="w-full border rounded p-2 bg-white dark:bg-gray-900" value="{{ old('reference', 'RCV-'.date('YmdHis')) }}" />
                </div>
                <div>
                    <label class="block text-sm">Supplier</label>
                    <input name="supplier" class="w-full border rounded p-2 bg-white dark:bg-gray-900" />
                </div>
            </div>
            ...
            <div>
                <button class="px-4 py-2 bg-erp text-white rounded">Save Receipt</button>
            </div>
        </form>
    </div>
        ...
        document.getElementById('add-item').addEventListener('click', function(){
            ...
            div.innerHTML = `
                <select name="items[${idx}][product_id]" class="border rounded p-2 bg-white dark:bg-gray-900">
                    <option value="">-- Select Product --</option>
                    ${(() => { let s=''; App\Models\Product::orderBy('name')->get().forEach(p=>{ s+=`<option value="${p.id}">${p.name}</option>`}); return s; })()}
                </select>
