...
@section('content')
    <div class="p-6">
        <h1 class="text-2xl font-semibold mb-4">New Transfer</h1>
        ...
        <form method="post" action="{{ route('admin.inventory.transfers.store') }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                    ...
                    <input name="reference" class="w-full border rounded p-2 bg-white dark:bg-gray-900" value="{{ old('reference', 'TRF-'.date('YmdHis')) }}" />
                </div>
                <div>
                    <label class="block text-sm">From location</label>
                    <input name="from_location" class="w-full border rounded p-2 bg-white dark:bg-gray-900" />
                </div>
                <div>
                    <label class="block text-sm">To location</label>
                    <input name="to_location" class="w-full border rounded p-2 bg-white dark:bg-gray-900" />
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                ...
                <div id="items">
                    <div class="grid grid-cols-4 gap-2 mb-2 item-row">
                        ...
                        <input name="items[0][qty]" placeholder="Qty" class="border rounded p-2 bg-white dark:bg-gray-900" />
            ...
            <div>
                <button class="px-4 py-2 bg-erp text-white rounded">Save Transfer</button>
            </div>
        </form>
    </div>
    ...
    (function(){
        ...
        document.getElementById('add-item').addEventListener('click', function(){
            ...
            div.innerHTML = `
                ...
                <input name="items[${idx}][qty]" placeholder="Qty" class="border rounded p-2 bg-white dark:bg-gray-900" />
