...
@section('content')
    <div class="p-6">
        <h1 class="text-2xl font-semibold mb-4">New Delivery</h1>

        <form method="post" action="{{ route('admin.inventory.deliveries.store') }}">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="block text-sm">Reference</label>
                    <input name="reference" class="w-full border rounded p-2 bg-white dark:bg-gray-900" value="{{ old('reference', 'DLV-'.date('YmdHis')) }}" />
                </div>
                <div>
                    <label class="block text-sm">Customer</label>
                    <input name="customer" class="w-full border rounded p-2 bg-white dark:bg-gray-900" />
                </div>
            </div>
            ...
            <div>
                <button class="px-4 py-2 bg-erp text-white rounded">Save Delivery</button>
            </div>
        </form>
    </div>
        ...
        document.getElementById('add-item').addEventListener('click', function(){
            ...
            div.innerHTML = `
                <select name="items[${idx}][product_id]" class="border rounded p-2 bg-white dark:bg-gray-900">
                    <option value="">-- Select Product --</option>
                    ${(() => { let s=''; /* server-side placeholder: list replaced at render */ return s; })()}
                </select>
